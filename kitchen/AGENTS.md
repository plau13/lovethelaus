...
- **Never fail silently.** A path that returns a status code and logs nothing is unobservable: `src/lib/log.ts` gives `logWarn`/`logError`, one JSON line with an `event` name, and `safeFields` redacts credential-shaped keys (booleans excepted — one bit is not a secret). Never pass a payload, signature or key. Events and the alert setup are listed in [`docs/CLOUDFLARE.md`](../docs/CLOUDFLARE.md).
- **Preferences configure something or they do not exist.** The four a cook sets — recipe box name, default servings, units, default cookbook visibility — are validated in one place, `src/lib/kitchen-prefs.ts`, shared by onboarding and Settings so the two forms cannot drift. Each one changes behaviour: the box name titles the default cookbook, servings prefill `RecipeEditor`, visibility seeds a new cookbook. A stored answer nothing reads is a survey question, which is what the nine-question onboarding interview was before it was removed.
